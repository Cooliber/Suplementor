...
import { IntakeLogSchema, type IntakeLog, type SupplementWithTracking } from '@/types/supplement';
...
export const IntakeDragBoard = () => {
  ...
  const updateSuggestions = (currentSupplements: SupplementWithTracking[]) => {
    try {
      ...
      const mappedSuggestions: Suggestion[] = result.recommendations.slice(0, 3).map(rec => ({
        item: {
      ...
      ErrorLogger.getInstance().log(new AppError(
          'Failed to update suggestions',
          ErrorType.COMPONENT,
          { error, currentSupplements }
        ));
      setSuggestions([]);
    }
  };
     ...
     if (savedLogs && Array.isArray(savedLogs)) {
       ...
       for (const log of savedLogs) {
         ...
         if (result.success) {
           ...
           ErrorLogger.getInstance().log(new AppError(
             'Invalid log format in storage',
             ErrorType.VALIDATION,
             { log, errors: result.error.errors }
           ));
         }
       }
  ...
  const addSupplement = () => {
    if (selectedSupplementId) {
      ...
      if (supToAdd) {
        setCurrentLog((prev) => {
          const newSupplements = [...prev.supplements, { ...supToAdd, dosage: '0', time: '' }];
          updateSuggestions(newSupplements);
          return {
            ...prev,
            supplements: newSupplements,
  ...
  const updateSupplement = (id: string, field: 'dosage' | 'time', value: string) => {
    setCurrentLog((prev) => ({
      ...prev,
      supplements: prev.supplements.map(s => s.id === id ? { ...s, [field]: value } : s),
    }));
  };
  ...
  const saveLog = () => {
    const immuneImpactValue = currentLog.supplements.reduce((sum, sup) => {
      ...
      if (dbSup) {
        ...
        const benefitScore = dbSup.benefits?.reduce((score: number, benefit: string) => {
          if (benefit.toLowerCase().includes('immune') || benefit.toLowerCase().includes('stress reduction') || benefit.toLowerCase().includes('antioxidant')) {
    ...
    setCurrentLog({ date: new Date().toISOString().split('T')[0] || '', supplements: [], immuneImpact: [] });
    ...
    <Card>
      ...
      <CardContent>
        <div className="mb-4 flex space-x-2">
          <Select onValueChange={setSelectedSupplementId} value={selectedSupplementId || ''}>
          ...
          <Button onClick={addSupplement} disabled={!selectedSupplementId}>Add</Button>
        </div>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd} modifiers={[restrictToVerticalAxis]}>
          <SortableContext items={currentLog.supplements.map(s => s.id)} strategy={verticalListSortingStrategy}>
            {currentLog.supplements.map((sup) => (
              <SortableItem key={sup.id} id={sup.id} supplement={sup} updateSupplement={updateSupplement} removeSupplement={removeSupplement} />
            ))}
          </SortableContext>
        </DndContext>
        <Button onClick={saveLog} className="mt-4">Zapisz Dziennik</Button>
        <div className="mt-6">
          <h3 className="text-lg font-semibold">Trend Wpływu na Odporność</h3>
          {logs.length > 0 && (
            <Sparkline data={logs.map(log => log.immuneImpact[0] || 0)} width={300} height={100} strokeColor="#3b82f6" strokeWidth={2} />
          )}
        </div>
                  ...
                  {suggestions.map((suggestion, index) => (
                    <Tooltip key={index}>
                      <TooltipTrigger asChild>
                        <div className="flex items-center p-2 bg-blue-50 rounded cursor-pointer hover:bg-blue-100">
                          <div className="font-medium mr-2">{suggestion.item.polishName}</div>
                          <Badge variant="secondary" className="ml-auto">{Math.round(suggestion.synergyScore)}%</Badge>
                        </div>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{suggestion.item.description}</p>
                        <p className="text-sm text-muted-foreground mt-1">Neuroefekty: {suggestion.item.neuroEffects.join(', ')}</p>
                      </TooltipContent>
                    </Tooltip>
                  ))}
                </div>
 ...
 *
 */
const SortableItem = ({ id, supplement, updateSupplement, removeSupplement }: { id: string; supplement: SupplementWithTracking; updateSupplement: (id: string, field: 'dosage' | 'time', value: string) => void; removeSupplement: (id: string) => void; }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center mb-2 p-2 border rounded">
