...
export const brainRegions: BrainRegionData[] = [
  ...
  },
  {
    ...
    description: 'Przetwarza emocje, szczególnie strach i agresję, ośrodek walki lub ucieczki',
...
const BrainMesh = ({ region, isSelected, onClick, animateRegionId }: BrainMeshProps) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      } else if (material && 'opacity' in material && typeof material.opacity === 'number') {
      ...
      const pulse = pulseActive ? 1 + 0.08 * (1 + Math.sin(state.clock.elapsedTime * 3)) * 0.5 : 1;
  ...
  return (
    ...
    >
      ...
      {(hovered || isSelected) && (
        <Html
          position={[region.position[0], region.position[1] + region.size + 0.1, region.position[2]]}
          center
          className="pointer-events-none"
        >
          <div className="bg-white border border-gray-300 rounded-lg p-2 shadow-lg max-w-48 text-center">
 ...
 *
 */
export default function Brain3D({ selectedRegion, onRegionClick, arMode = false, animateRegionId = null }: Brain3DProps) {
  ...
  return (
    <div className="w-full h-96 bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg overflow-hidden relative" aria-label="Interaktywny model mózgu">
      <Canvas camera={{ position: [0, 0, 3], fov: arMode ? 75 : 60 }} onCreated={({ gl }) => {
        ...
        <group>
          ...
          {brainRegions.map((region) => (
            <BrainMesh
              key={region.id}
              region={region}
              isSelected={selectedRegion?.id === region.id || internalSelected?.id === region.id}
        ...
        <OrbitControls enableZoom={true} enablePan={false} maxDistance={5} minDistance={1.5} enabled={!arMode} />
      </Canvas>

      {/* Instructions */}
      {!arMode ? (
        <div className="absolute bottom-4 left-4 text-xs text-gray-600 bg-white/80 px-3 py-2 rounded">
          Kliknij region mózgu aby zobaczyć szczegóły • Przeciągnij aby obrócić • Scroll aby przybliżyć
        </div>
      ) : (
        <div className="absolute bottom-4 left-4 text-xs text-green-800 bg-green-50/90 border border-green-200 px-3 py-2 rounded">
          Tryb AR: podgląd symulowany. Pełna obsługa WebXR zostanie włączona na urządzeniach zgodnych i w bezpiecznym połączeniu (HTTPS).
        </div>
      )}
    </div>
  );
