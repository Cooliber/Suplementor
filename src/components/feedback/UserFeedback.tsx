...
export const useSupplementFeedback = () => {
  ...
  const intakeLogged = useCallback(
  ...
  const intakeUpdated = useCallback(
    (supplementName: string) => {
  ...
  const doseWarning = useCallback(
    (supplementName: string, maxDose: number) => {
      showWarning(`Uwaga: przekroczono zalecaną dawkę ${supplementName} (${maxDose})`)
    },
    [showWarning]
  )

  const interactionWarning = useCallback(
    (supplements: string[]) => {
  ...
  const reminderSet = useCallback(
    (supplementName: string, time: string) => {
      showSuccess(`Przypomnienie o ${supplementName} ustawione na ${time}`)
    },
    [showSuccess]
  )

  const streakAchieved = useCallback(
    (days: number) => {
  ...
  const complianceImproved = useCallback(
    (percentage: number) => {
