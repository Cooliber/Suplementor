...
const AdvancedAnalytics = ({ stats, supplements }: AdvancedAnalyticsProps) => {
  ...
  const supplementEffectiveness = useMemo(() => {
    const categories = supplements.reduce(
      (acc, supplement) => {
        const category = supplement.description.includes('pamięć')
          ? 'Pamięć'
          : supplement.description.includes('energia')
            ? 'Energia'
            : supplement.description.includes('koncentracja')
              ? 'Koncentracja'
              : supplement.description.includes('sen')
                ? 'Sen'
                : 'Inne'
  ...
  const complianceData = useMemo(
    () => [
      ...
      {
        name: 'Pozostałe',
        value: stats.supplementsToday.total - stats.supplementsToday.taken,
        color: '#F59E0B'
      }
    ],
                ...
                <div key={metric.name} className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <Icon className={`mr-2 h-4 w-4 ${metric.color}`} />
            ...
            <div className="space-y-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">
                  {Math.round(
                    (stats.supplementsToday.taken / stats.supplementsToday.total) * 100
                  )}
                  %
                </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {insights.map((insight, index) => {
              ...
              return (
                <div key={index} className={`rounded-lg border p-4 ${bgColor}`}>
                  <div className="flex items-start">
                    <AlertCircle className={`mt-0.5 mr-3 h-5 w-5 ${iconColor}`} />
