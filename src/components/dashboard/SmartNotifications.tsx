...
const SmartNotifications = ({
  ...
  const generateNotifications = useMemo(() => {
    ...
    if (settings.supplementReminders) {
      supplements.forEach((supplement) => {
        if (supplement.status !== 'taken') {
          ...
          const isUpcoming =
            isBefore(now, scheduledTime) && isAfter(now, addHours(scheduledTime, -1))
    ...
    if (settings.healthWarnings) {
      ...
      const hasOmega3 = supplements.some(
        (s) => s.name.includes('Omega-3') && s.status === 'taken'
      )
      const hasLTheanine = supplements.some(
        (s) => s.name.includes('L-Theanine') && s.status === 'taken'
      )
      ...
      if (hasOmega3 && hasLTheanine) {
        ...
        if (timeDiff < 2) {
          newNotifications.push({
            ...
            message:
              'L-Theanine z kofeiną najlepiej działa na pusty żołądek, 2h po Omega-3',
            time: now,
            priority: 'low',
            actionable: false
          })
      ...
      <CardContent>
        {showSettings && (
          <div className="mb-6 rounded-lg border bg-gray-50 p-4">
            <h4 className="mb-3 font-medium">Ustawienia powiadomień</h4>
        ...
        <div className="space-y-3">
          {activeNotifications.length === 0 ? (
            ...
            activeNotifications.map((notification) => (
              <div
                key={notification.id}
                className={`rounded-lg border p-4 ${getPriorityColor(notification.priority)}`}
              >
