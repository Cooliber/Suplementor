    ...
    try {
      ...
      if (typeof result !== 'string') {
        throw new StorageError(
          'Data cannot be serialized to string',
          'SERIALIZE_ERROR'
        );
      }
      return result;
...
export class SupplementDataManager {
  ...
  async addSupplement(supplement: BaseSupplement): Promise<StorageResult<BaseSupplement>> {
    const validation = validateSupplement(supplement);
  ...
  async updateSupplement(id: string, updates: Partial<BaseSupplement>): Promise<StorageResult<BaseSupplement>> {
    const current = await this.getSupplements();
    if (!current.success) {
      return current;
    }
  ...
  async getSupplementLogs(supplementId?: string): Promise<StorageResult<SupplementLog[]>> {
  ...
  async setAIRecommendations(recommendations: AIRecommendation[]): Promise<StorageResult<void>> {
    const saveResult = await this.storage.set(STORAGE_KEYS.AI_RECOMMENDATIONS, recommendations);
    if (saveResult.success) {
  ...
  async addPerformanceMetric(metric: PerformanceMetric): Promise<StorageResult<PerformanceMetric>> {
