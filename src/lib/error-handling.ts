 ...
 * @param schema
 */
export const safeJsonParse = <T>(jsonString: string, schema?: z.ZodSchema<T>): { success: true; data: T } | { success: false; error: AppError } => {
  try {
    ...
    try {
      ...
      ErrorLogger.getInstance().log(
        new AppError(
          ...
          ErrorType.STORAGE,
          { key, value, error } as Record<string, unknown>
        )
      );
      return false;
    }
 ...
 * @param schema
 */
export const validateSupplementData = <T>(data: unknown, schema: z.ZodSchema<T>): { success: true; data: T } | { success: false; errors: string[] } => {
    ...
    override componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
      ErrorLogger.getInstance().log(
        new AppError(
          ...
          ErrorType.COMPONENT,
          { error, errorInfo } as Record<string, unknown>
        )
      );
    }
 ...
 * @param asyncFn
 */
export const safeAsync = async <T>(asyncFn: () => Promise<T>): Promise<{ success: true; data: T } | { success: false; error: AppError }> => {
  try {
