...
import { supplementInteractionChecker, type InteractionAnalysis } from './supplement-interaction-checker';
...
class AIRecommendationEngine {
  ...
  private initializeGoalProfiles() {
    ...
    this.goalProfiles.set(UserGoal.COGNITIVE_ENHANCEMENT, {
      ...
      supportingSupplements: ['omega-3', 'magnesium-l-threonate', 'phosphatidylserine', 'uridine'],
    ...
    this.goalProfiles.set(UserGoal.ENERGY_BOOST, {
      ...
      targetSystems: [BodySystem.NERVOUS, BodySystem.ENDOCRINE, BodySystem.CARDIOVASCULAR],
      keyMechanisms: [
    ...
    this.goalProfiles.set(UserGoal.ATHLETIC_PERFORMANCE, {
      ...
      targetSystems: [BodySystem.MUSCULOSKELETAL, BodySystem.CARDIOVASCULAR, BodySystem.NERVOUS],
      keyMechanisms: [
        MechanismOfAction.METABOLIC_ENHANCEMENT,
        MechanismOfAction.ANTIOXIDANT,
        MechanismOfAction.ANTI_INFLAMMATORY
    ...
    for (const supplement of availableSupplements) {
      ...
      if (overallScore > 30) { // Minimum threshold
        const recommendation: SupplementRecommendation = {
          ...
          supplement,
          score: overallScore,
          reasoning: this.generateReasoning(supplement, goalProfile, relevanceScore, evidenceScore),
          warnings: safetyAnalysis.warnings.map(w => w.description),
          dosageRecommendation: this.calculatePersonalizedDosage(supplement, userProfile, goal),
   ...
   */
  private calculateGoalRelevance(supplement: EnhancedSupplement, goalProfile: GoalProfile): number {
    let score = 0;
    let maxScore = 0;
    ...
    const primaryEffectMatch = supplement.primaryEffects.some(effect =>
      goalProfile.primarySupplements.some(primary => effect.toLowerCase().includes(primary))
    );
  ...
  private generateReasoning(
    ...
    if (supplement.userGoals.includes(goalProfile.goal)) {
      reasoning.push(`Directly targets your goal of ${goalProfile.goal.replace('_', ' ')}`);
    }
   ...
   */
  private calculateCostEffectiveness(supplement: EnhancedSupplement, score: number): number {
    if (!supplement.costPerDose || !supplement.costEffectiveness) return 50;

    const costMultiplier = { high: 1.2, medium: 1.0, low: 0.8 }[supplement.costEffectiveness];
  ...
  private applyRecommendationFilters(
    ...
    return recommendations.filter(rec => {
      // Evidence threshold
      if (criteria.evidenceThreshold && rec.evidenceScore < this.getEvidenceThreshold(criteria.evidenceThreshold)) {
        return false;
      }

      // Safety threshold
      if (criteria.safetyThreshold && rec.supplement.evidenceLevel === EvidenceLevel.INSUFFICIENT) {
        return false;
      }

      // Avoid ingredients
      if (criteria.avoidIngredients?.some(ingredient =>
      ...
      if (criteria.preferredFormulations?.length && 
      ...
      // Budget constraints
      if (criteria.budget && rec.supplement.costPerDose && 
  ...
  public async optimizeSupplementStack(
    ...
    const currentStackAnalysis = this.analyzeCurrentStack(userProfile, availableSupplements);
    ...
    const costAnalysis = this.calculateCostAnalysis(userProfile, newRecommendations, availableSupplements);
  ...
  private identifyDosageAdjustments(
    ...
    userProfile.currentSupplements.forEach(current => {
      ...
      const currentDosage = currentDosageMatch ? parseFloat(currentDosageMatch[1]) : standardMin;

      // Suggest adjustments based on effectiveness and side effects
      if (current.effectiveness && current.effectiveness < 5 && currentDosage < standardMax) {
        adjustments.push({
          supplementId: current.supplementId,
