...
class ResearchIntegrationSystem {
  ...
  public async searchPubMed(
    ...
    try {
      ...
      if (!searchData.esearchresult?.idlist?.length) {
        advancedDebugger.warn(DebugCategory.API, 'No results found for PubMed search', {
          query
        })
        return []
      }
  ...
  public async fetchAbstract(pmid: string): Promise<string | null> {
    try {
      ...
      advancedDebugger.error(DebugCategory.API, 'Failed to fetch abstract', {
        pmid,
        error
      })
      return null
   ...
   */
  public async generateSupplementResearchSummary(
    ...
    const uniqueResults = allResults.filter(
      (result, index, array) => array.findIndex((r) => r.pmid === result.pmid) === index
    )
  ...
  public analyzeEvidence(
    ...
    const overallStrength = this.determineOverallEvidenceLevel(
      confidenceScore,
      supplement.research
    )
    ...
    return {
      ...
      recommendations: this.generateEvidenceRecommendations(
        confidenceScore,
        overallStrength
      )
    }
  ...
  private extractDosageRecommendations(
    supplement: EnhancedSupplement,
    studies: PubMedSearchResult[]
  ): string[] {
  ...
  private calculateConsistencyScore(research: ResearchStudy[]): number {
    ...
    const moderateCount = evidenceLevels.filter(
      (e) => e === EvidenceLevel.MODERATE
    ).length
    const conflictingCount = evidenceLevels.filter(
      (e) => e === EvidenceLevel.CONFLICTING
    ).length
  ...
  private determineOverallEvidenceLevel(
    confidenceScore: number,
    research: ResearchStudy[]
  ): EvidenceLevel {
    if (
      confidenceScore >= 85 &&
      research.some((r) => r.evidenceLevel === EvidenceLevel.STRONG)
    ) {
  ...
  private generateEvidenceRecommendations(
    ...
    if (evidenceLevel === EvidenceLevel.CONFLICTING) {
      recommendations.push(
        'Results are mixed - individual response may vary significantly'
      )
    }
