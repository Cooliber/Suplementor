...
const StackOptimizationSchema = z.object({
  ...
  costOptimization: z.object({
    ...
    costEffectiveAlternatives: z.array(z.string()).describe('More cost-effective alternatives')
  }),
  timeline: z.object({
...
const PersonalizedRecommendationSchema = z.object({
  ...
  lifestyle_recommendations: z.array(z.object({
    category: z.enum(['diet', 'exercise', 'sleep', 'stress_management', 'other']),
    recommendation: z.string(),
    impact: z.enum(['high', 'medium', 'low']),
    difficulty: z.enum(['easy', 'moderate', 'challenging'])
  })),
  monitoring_suggestions: z.array(z.object({
    parameter: z.string(),
    frequency: z.string(),
    method: z.string(),
  ...
  personalization_notes: z.string().describe('Specific notes about why these recommendations fit this user')
});
...
export class AIKnowledgeEngine {
  ...
  private constructor() {
    advancedDebugger.info(DebugCategory.AI, 'AI Knowledge Engine initialized with Gemini Flash');
  }
  ...
  public async optimizeSupplementStack(
    ...
    const prompt = this.buildStackOptimizationPrompt(userProfile, currentSupplements, availableSupplements);
  ...
  public async answerKnowledgeQuery(
    ...
    if (context?.includeResearch) {
      try {
        ...
        advancedDebugger.warn(DebugCategory.AI, 'Research integration failed, continuing without', error);
      }
    }
  ...
  public async generatePersonalizedRecommendations(
    ...
    try {
      ...
      advancedDebugger.error(DebugCategory.AI, 'Personalized recommendations failed', error);
  ...
  public async createInteractiveChat(
    ...
    const history = context?.conversationHistory ? this.conversationHistory.get(userId) || [] : [];
      ...
      const result = await streamText({
        ...
        tools: {
          analyzeSupplementInteraction: tool({
            ...
            parameters: z.object({
              supplements: z.array(z.string()).describe('List of supplement names to analyze')
            }),
          ...
          searchResearch: tool({
            description: 'Search for latest research on a specific supplement or health topic',
            parameters: z.object({
          ...
          recommendSupplements: tool({
            ...
            parameters: z.object({
              ...
              restrictions: z.array(z.string()).optional().describe('Any restrictions or preferences')
            }),
  ...
  public async generateEducationalContent(
    ...
    Structure the response as ${format === 'article' ? 'a full article with sections' : 
                              format === 'summary' ? 'key points and takeaways' :
                              format === 'qa' ? 'questions and detailed answers' :
    ...
    try {
      ...
      advancedDebugger.error(DebugCategory.AI, 'Educational content generation failed', error);
