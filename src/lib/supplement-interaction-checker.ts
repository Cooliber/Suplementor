...
class SupplementInteractionChecker {
    ...
    const knownInteractions: DetailedInteraction[] = [
      {
        ...
        description:
          'Alpha-GPC and Citicoline work synergistically to enhance cholinergic function',
      ...
      },
      {
        ...
        description:
          'L-theanine smooths caffeine effects and improves focus without jitters',
      ...
      },
      {
        ...
        clinicalEvidence: EvidenceLevel.STRONG,
        description:
          'Black pepper extract (piperine) increases curcumin absorption by 2000%',
    ...
    userProfile.healthConditions.forEach((condition) => {
      const contraindicated =
        conditionContraindications[
          condition.toLowerCase() as keyof typeof conditionContraindications
        ]
      if (contraindicated) {
  ...
  private calculateRiskLevel(
    ...
    const severeCounts = {
      severe: interactions.filter((i) => i.severity === InteractionSeverity.SEVERE)
        .length,
      ...
      beneficial: interactions.filter(
  ...
  private generateRecommendations(
    ...
    if (timingInteractions.length > 0) {
      ...
      timingInteractions.forEach((interaction) => {
        recommendations.push(
          `Take ${interaction.supplement1} and ${interaction.supplement2} ${interaction.timeGap} apart`
        )
      })
    }
    ...
    if (userProfile.medications.length > 0) {
      monitoringRequired.push(
        'Regular monitoring recommended due to medication interactions'
      )
    }
      ...
      supplements: [
        {
          id: supplementId,
          dosage:
            supplement.dosageInfo.standard.min + supplement.dosageInfo.standard.unit,
          timing: 'morning'
        },
