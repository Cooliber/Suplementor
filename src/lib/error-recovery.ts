...
rax.attach(retryClient, retryConfig)
...
class ServiceHealthMonitor {
  ...
  private initializeCircuitBreakers() {
    ...
    services.forEach((service) => {
      const breaker = new CircuitBreaker(() => this.performHealthCheck(service), {
...
export class ErrorRecoveryManager {
  ...
  async executeWithRecovery<T>(
    ...
    const { service, retries = 3, backoff = 1000, fallback, timeout = 5000 } = options
  ...
  async getRecoveryStatus() {
    ...
    const circuitStates = Array.from(this.healthMonitor.circuitBreakers.entries()).reduce(
      (acc, [service, breaker]) => {
        acc[service] = {
    ...
    return {
      ...
      overallHealth: services.every((s) => s.status === 'healthy')
        ? 'healthy'
  ...
  static async retryConnection(retries = 3, delay = 1000): Promise<boolean> {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        logger.warn(`Database connection failed, attempt ${attempt}/${retries}`, {
          error
        })
