...
class AdvancedDebugger {
  ...
  private constructor() {
    ...
    this.isEnabled = process.env.NODE_ENV === 'development' || localStorage.getItem('debug-mode') === 'true';
    this.loadFilters();
  }
  ...
  private loadFilters() {
    try {
      ...
      if (savedFilters) {
        this.filters = new Set(JSON.parse(savedFilters));
      }
    } catch (error) {
  ...
  log(level: DebugLevel, category: DebugCategory, message: string, data?: any, userId?: string) {
    ...
    if (level <= DebugLevel.WARN) {
      clientLogger.warn(`[${category}] ${message}`, { data, userId, sessionId: this.sessionId });
    }
  }
...
export function useComponentDebugger(componentName: string) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      debuggerInstance.debug(DebugCategory.RENDERING, `${componentName} unmounted after ${lifetime}ms`);
    };
  }, [componentName, debuggerInstance]);

  useEffect(() => {
    renderCountRef.current++;
    debuggerInstance.trace(DebugCategory.RENDERING, `${componentName} rendered (${renderCountRef.current})`);
  });

  return {
    logInfo: useCallback((message: string, data?: any) => {
      debuggerInstance.info(DebugCategory.USER_INTERACTION, `[${componentName}] ${message}`, data);
    }, [componentName, debuggerInstance]),

    logError: useCallback((message: string, error?: Error) => {
      debuggerInstance.error(DebugCategory.ERROR, `[${componentName}] ${message}`, { error: error?.message, stack: error?.stack });
    }, [componentName, debuggerInstance]),

    logPerformance: useCallback((operation: string, duration: number) => {
      debuggerInstance.info(DebugCategory.PERFORMANCE, `[${componentName}] ${operation} took ${duration}ms`);
    }, [componentName, debuggerInstance]),
  };
}
...
export function usePerformanceMonitor(operationName: string) {
  ...
  const end = useCallback((additionalData?: any) => {
    if (startTimeRef.current) {
      const duration = performance.now() - startTimeRef.current;
      debuggerInstance.info(DebugCategory.PERFORMANCE, `${operationName} completed in ${duration.toFixed(2)}ms`, {
        duration,
        ...additionalData,
      });
      return duration;
...
export const supplementDebugger = {
  logInteraction: (supplementId: string, interactionType: 'take' | 'skip' | 'modify', data?: any) => {
    const debuggerInstance = AdvancedDebugger.getInstance();
    debuggerInstance.info(DebugCategory.SUPPLEMENT, `Supplement ${interactionType}: ${supplementId}`, data);
  },

  logEffect: (supplementId: string, effectType: 'positive' | 'negative' | 'neutral', description: string, severity: number) => {
    const debuggerInstance = AdvancedDebugger.getInstance();
    debuggerInstance.info(DebugCategory.SUPPLEMENT, `Supplement effect: ${supplementId}`, {
      effectType,
      description,
      severity,
      timestamp: new Date(),
  ...
  logInteractionWarning: (supplement1: string, supplement2: string, severity: 'low' | 'medium' | 'high', description: string) => {
    const debuggerInstance = AdvancedDebugger.getInstance();
    debuggerInstance.warn(DebugCategory.SUPPLEMENT, `Supplement interaction warning: ${supplement1} + ${supplement2}`, {
      severity,
      description,
