...
export default function StackBuilderPage() {
  ...
  const getItemById = (id: string): Supplement | null =>
    [...nootropicsDatabase, ...supplementsDatabase].find(
      (s) => s.id === id || s.name === id
    ) ?? null
  ...
  const handleJsonExport = () => {
    const stackData = stack
      ...
      .filter(Boolean)
    const blob = new Blob([JSON.stringify(stackData, null, 2)], {
  ...
  return (
    <div className="container mx-auto space-y-6 p-4">
      ...
      <div className="flex flex-col gap-6 sm:flex-col md:flex-row">
                    ...
                    {nootropics.map((nootropic) => (
                      <SortableItem key={nootropic.id} id={nootropic.id}>
                        <Card
                          ...
                          aria-label={`Przeciągnij nootropik ${nootropic.polishName}`}
                        >
                          <p className="font-medium">
                            {nootropic.polishName} ({nootropic.name})
                          </p>
                        </Card>
                      </SortableItem>
                    ))}
            ...
            <CardContent>
                  ...
                  {stack.map((id) => {
                    ...
                    return (
                      <SortableItem key={id} id={id}>
                        <Card
                          className="p-2"
                          aria-label={`Element stacku ${item.polishName}`}
                        >
              ...
              <div className="mt-4">
                ...
                <Button
                  ...
                  aria-label="Eksportuj stos do PDF"
                  className="ml-2"
                >
                  Eksportuj stos (PDF)
                </Button>
        ...
        <CardContent>
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="układ nerwowy">
                Układ nerwowy (Nervous System) - Wsparcie poznawcze
              </SelectItem>
              <SelectItem value="układ krążenia">
                Układ krążenia (Cardiovascular) - Wsparcie serca
              </SelectItem>
              <SelectItem value="układ odpornościowy">
                Układ odpornościowy (Immune) - Wzmocnienie obrony
              </SelectItem>
              <SelectItem value="układ trawienny">
                Układ trawienny (Digestive) - Wsparcie jelit
              </SelectItem>
              <SelectItem value="układ endokrynny">
                Układ endokrynny (Endocrine) - Balans hormonalny
              </SelectItem>
              <SelectItem value="układ mięśniowo-szkieletowy">
                Układ mięśniowo-szkieletowy (Musculoskeletal) - Siła i kości
              </SelectItem>
            </SelectContent>
          </Select>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {goals.map((goal, index) => (
                <Badge
                  ...
                  onClick={() => setGoals((prev) => prev.filter((g) => g !== goal))}
                >
                  {goal} ✕
                </Badge>
              ))}
      ...
      {recommendations.length > 0 && (
        <div>
          <h2 className="mb-4 text-2xl font-semibold">Sugerowane Suplementy</h2>
              ...
              return (
                <Card key={index}>
                  ...
                  <CardContent>
                    ...
                    <p className="mt-2 text-sm">Dawkowanie: {dosage}</p>
                    {validationResult?.systemWarnings &&
                      validationResult.systemWarnings.length > 0 && (
