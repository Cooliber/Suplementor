...
const predefinedSynergies: {
  [key: string]: { pair: string; score: number; goal: string }[]
} = {
  'L-Theanine': [
...
export const recommendStack = (
  ...
  if (userProfile?.age && userProfile.age > 60) {
    candidates = candidates.filter((item: SupplementItem) => {
      ...
      safetyText = item.sideEffects?.join(' ').toLowerCase() || ''
      return (
        !safetyText.includes('tolerancję') &&
        !safetyText.includes('uzależnienie') &&
        !safetyText.includes('tolerance') &&
        !safetyText.includes('addiction')
      )
    })
  }
  ...
  if (goals.some((g) => g.startsWith('układ'))) {
    ...
    candidates.forEach((item: SupplementItem) => {
      ...
      if (entry && entry.system === systemGoal.replace('układ ', '').replace('owy', '')) {
        if (currentStack) {
          const incompatMatches = entry.incompatibleWith.filter((inc: string) =>
            currentStack.includes(inc)
          )
          if (incompatMatches.length > 0) {
            systemWarnings.push(
              `Ostrzeżenie: Interakcja w ${systemGoal} (Warning: Interaction in ${systemGoal}) - ${itemName} z ${incompatMatches.join(', ')}`
            )
          }
        }
      }
    ...
    if (currentStack) {
      if (entry) {
        ...
        if (incompatMatches.length > 0) {
          ...
          localWarnings.push(
            `Potencjalna interakcja: ${itemName} z ${incompatMatches.join(', ')} - ryzyko wysokiego poziomu`
          )
        }
        // Check synergies
        const synergyMatches = entry.synergyWith.filter((syn: string) =>
          currentStack.includes(syn)
        )
        interactionScore += 20 * synergyMatches.length
        if (synergyMatches.length > 0) {
          localWarnings.push(
            `Synergia: ${itemName} z ${synergyMatches.join(', ')} - +20 punktów`
          )
        }
      ...
      for (let j = i + 1; j < currentStack.length; j++) {
        ...
        if (item1 && (item1 as keyof typeof interactionsMatrix) in interactionsMatrix) {
          ...
          if (item2 && entry1.incompatibleWith.includes(item2)) {
            warnings.push(
              `Ostrzeżenie: ${item1} i ${item2} - niezgodność (incompatible), ryzyko ${entry1.riskLevel}`
            )
        ...
        if (item2 && (item2 as keyof typeof interactionsMatrix) in interactionsMatrix) {
          ...
          if (item1 && entry2.incompatibleWith.includes(item1)) {
            warnings.push(
              `Ostrzeżenie: ${item2} i ${item1} - niezgodność (incompatible), ryzyko ${entry2.riskLevel}`
            )
