...
export const StackProgress = ({ stackId, stackName, totalDays }: StackProgressProps) => {
  ...
  const [supplements, setSupplements] = useState<string[]>([
    'L-teanina', 'Magnez L-treonian', 'Alfa-GPC', 'Różeniec górski', 'Ashwagandha'
  ]);
  ...
  const calculateAverages = (logs: DailyLog[]) => {
    if (logs.length === 0) return { sleepQuality: 0, energyLevel: 0, focusLevel: 0, moodLevel: 0, stressLevel: 0 };
  ...
  const calculateImprovements = (logs: DailyLog[]) => {
    if (logs.length < 7) return { sleepQuality: 0, energyLevel: 0, focusLevel: 0, moodLevel: 0, stressLevel: 0 };
    ...
    return {
      sleepQuality: Math.round((lastWeekAvg.sleepQuality - firstWeekAvg.sleepQuality) * 10) / 10,
      energyLevel: Math.round((lastWeekAvg.energyLevel - firstWeekAvg.energyLevel) * 10) / 10,
      focusLevel: Math.round((lastWeekAvg.focusLevel - firstWeekAvg.focusLevel) * 10) / 10,
      moodLevel: Math.round((lastWeekAvg.moodLevel - firstWeekAvg.moodLevel) * 10) / 10,
      stressLevel: Math.round((firstWeekAvg.stressLevel - lastWeekAvg.stressLevel) * 10) / 10
    };
  };
  ...
  const getProgressPercentage = () => Math.min((progress.currentDay / totalDays) * 100, 100);
  ...
  return (
    <TooltipProvider>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>{stackName} - Dzień {progress.currentDay} z {totalDays}</CardDescription>
            </div>
        ...
        <p className="text-sm text-muted-foreground mt-2">{getProgressPercentage().toFixed(1)}% ukończone</p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          { label: 'Jakość snu', value: progress.averageMetrics.sleepQuality, improvement: progress.improvements.sleepQuality, icon: '😴' },
          { label: 'Energia', value: progress.averageMetrics.energyLevel, improvement: progress.improvements.energyLevel, icon: '⚡' },
          { label: 'Skupienie', value: progress.averageMetrics.focusLevel, improvement: progress.improvements.focusLevel, icon: '🎯' },
          { label: 'Nastrój', value: progress.averageMetrics.moodLevel, improvement: progress.improvements.moodLevel, icon: '😊' },
          { label: 'Stres', value: progress.averageMetrics.stressLevel, improvement: progress.improvements.stressLevel, icon: '😰', inverse: true }
        ].map(metric => (
          <Card key={metric.label}>
            <CardContent className="p-4">
              <div className="text-center">
                <div className="text-2xl mb-2">{metric.icon}</div>
                <div className={`text-2xl font-bold ${getMetricColor(metric.value, metric.inverse)}`}>
                  {metric.value.toFixed(1)}/10
                </div>
                <div className="text-sm text-muted-foreground">{metric.label}</div>
                {metric.improvement !== 0 && (
                  <div className={`text-xs ${getImprovementColor(metric.improvement, metric.inverse)}`}>
      ...
      <Card>
        ...
        <CardContent>
          {progress.dailyLogs.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">Brak wpisów. Dodaj swój pierwszy log!</p>
          ) : (
            <div className="space-y-3">
              {progress.dailyLogs.slice(-5).reverse().map(log => (
                <div key={log.id} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-medium">{dayjs(log.date).format('DD.MM.YYYY')}</span>
                    <TooltipProvider>
                      <div className="flex space-x-2 text-sm">
                        <Tooltip>
                          <TooltipTrigger>
                            <span className={cn("cursor-help", getMetricColor(log.sleepQuality))}>😴 {log.sleepQuality}/10</span>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>Jakość snu: Ocena regeneracji nocnej na podstawie głębokości snu i czasu trwania</p>
                          </TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger>
                            <span className={cn("cursor-help", getMetricColor(log.energyLevel))}>⚡ {log.energyLevel}/10</span>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>Poziom energii: Subiektywna ocena witalności i gotowości do działania przez cały dzień</p>
                          </TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger>
                            <span className={cn("cursor-help", getMetricColor(log.focusLevel))}>🎯 {log.focusLevel}/10</span>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>Skupienie: Zdolność do koncentracji i utrzymania uwagi na zadaniach poznawczych</p>
                          </TooltipContent>
                        </Tooltip>
                      </div>
                    </TooltipProvider>
