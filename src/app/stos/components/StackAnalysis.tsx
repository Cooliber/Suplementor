...
import { AlertTriangle, CheckCircle, Clock, Heart, Brain, Shield, Zap, AlertOctagon } from 'lucide-react';
...
const cognitiveGoals = ['pamięć', 'koncentracja', 'kognitywne', 'uczenie', 'energia neuronalną', 'plastyczność'];
...
const interactionDatabase: Record<string, InteractionRule> = {
  'stimulant-combo': {
    ...
    message: 'Kombinacja stymulantów może powodować nadmierną stymulację i problemy ze snem',
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
          ...
          const hasInteraction = supp1.interactions?.includes(supp2.name) || 
      ...
      items.forEach(item => {
        ...
        if (customNumbers.length > 0 && recommendedNumbers.length > 0) {
          ...
          if (maxCustom > maxRecommended * 1.5) { // 50% higher than recommended
            warnings.push(
      ...
      if (interactions.some(i => i.severity === 'high') || contraindications.length > 0) {
        overallRisk = 'high';
      } else if (interactions.some(i => i.severity === 'medium') || warnings.length > 3) {
        overallRisk = 'medium';
      }
      ...
      ErrorLogger.getInstance().log(new AppError(
        'Failed to calculate safety profile',
        ErrorType.COMPONENT,
        { error, items }
      ));
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge variant={safetyProfile.overallRisk === 'low' ? 'secondary' : safetyProfile.overallRisk === 'medium' ? 'default' : 'destructive'} className="flex items-center">
              {getRiskIcon(safetyProfile.overallRisk)}
              <span className="ml-2">
                {safetyProfile.overallRisk === 'low' ? 'Niskie ryzyko' :
                 safetyProfile.overallRisk === 'medium' ? 'Średnie ryzyko' : 'Wysokie ryzyko'}
              </span>
            </Badge>
          </div>
        </CardHeader>
          ...
          <div className="space-y-3">
            {safetyProfile.interactions.map((interaction, index) => (
              <div key={index} className={`border-l-4 rounded p-4 ${
                interaction.type === 'danger' ? 'border-red-500 bg-red-50' :
                interaction.type === 'warning' ? 'border-yellow-500 bg-yellow-50' :
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className={`text-sm font-medium ${
                      interaction.type === 'danger' ? 'text-red-800' :
                      interaction.type === 'warning' ? 'text-yellow-800' :
                      'text-blue-800'
                    }`}>
                      {interaction.message}
                    </p>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 rounded p-4">
            <h4 className="font-medium text-blue-900 mb-2">Rano (6-9h)</h4>
          ...
          <div className="bg-green-50 rounded p-4">
            <h4 className="font-medium text-green-900 mb-2">Popołudnie (12-15h)</h4>
          ...
          <div className="bg-purple-50 rounded p-4">
            <h4 className="font-medium text-purple-900 mb-2">Wieczór (18-21h)</h4>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-medium mb-2">Plan cykliczny</h4>
            <ul className="space-y-1 text-sm text-muted-foreground">
              {safetyProfile.cycleRecommendations.map((rec, index) => (
                <li key={index} className="flex items-start">
                  <Zap className="h-3 w-3 mr-2 mt-0.5 text-yellow-500" />
                  {rec}
                </li>
              ))}
            </ul>
          ...
          <div>
            <h4 className="font-medium mb-2">Monitorowanie efektów</h4>
            <ul className="space-y-1 text-sm text-muted-foreground">
        ...
        <CardContent>
          <div className="flex flex-col items-center space-y-4">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">Punkty synergii poznawczej</p>
            </div>
            <Sparkline
            ...
            <p className="text-sm text-muted-foreground text-center max-w-md">
              Wykres pokazuje potencjalny trend efektów synergicznych. Wyższy wynik oznacza lepszą synergię dla funkcji poznawczych.
            </p>
          </div>
        </CardContent>
      </Card>
